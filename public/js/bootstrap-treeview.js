...
;(function ($, window, document, undefined) {
	...
	Tree.prototype._subscribeEvents = function () {
		...
		this.$element.on('mouseover', $.proxy(this._hoverHandler, this));
		...
		if (typeof (this._options.onNodeHovered) === 'function') {
			this.$element.on('nodeHovered', this._options.onNodeHovered);
		}
	...
	*/
	...
	Tree.prototype._hoverHandler = function (event) {
		var target = $(event.target);

		if (target.closest('li.list-group-item').length > 0) {
			var node = this.targetNode(target);
			this._triggerEvent('nodeHovered', node, _default.options);
		}
	}
